...
const ProductAdvertisement: React.FunctionComponent<Props> = ({
  ...
  return (
    <View
      ...
      style={[
        style.newItemBox,
        { shadowColor: theme.labelBgColor, backgroundColor: theme.appColor },
      ]}
    >
      <Image style={[style.newItem]} source={imageUrl} />
      {label1 || label2 ? (
        <View style={[style.itemTitleLabel]}>
          <Text style={[style.itemTitleText, { color: theme.labelBgColor }]}>
            {label1}
          </Text>
          <Text style={[style.itemTitleText, { color: theme.labelBgColor }]}>
            {label2}
      ...
      {labelBuy ? (
        <TouchableOpacity
          style={[
            style.itemTitleView,
            { backgroundColor: theme.buyButtonLink },
          ]}
          onPress={onPress}
        >
          <Text style={[style.linkText, { color: theme.highlightTextColor }]}>
            {labelBuy}
          </Text>
        </TouchableOpacity>
      ) : null}
